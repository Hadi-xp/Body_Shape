...
const API = 'http://localhost:3000/getSize';

// const result = document.createElement('div').classList.add('result');
// const text = document.createElement('div').classList.add('text');
const div = document.querySelector('.text');
const p = document.createElement('p');
p.classList.add('data');
div.append(p)
// const p1 = document.createElement('div').classList.add('do');
// const p2 = document.createElement('div').classList.add('avoid');



// calculate body shape

async function bodyShapeCalculate(e){
    e.preventDefault();
    ...
    if (BHR < 0.95 && WHR < 0.8 && HHR < 0.95) {
        ...
        div.append(p)
        p.textContent = 'Your body shape is pear In this body shape, the hips are wider than the shoulders and chest. Most of the weight is in the lower body (hips, thighs, and bottom). The upper body looks smaller and slimmer compared to the lower body'
        ...
        const avoid = document.querySelector('.avoid');
        const shape = document.querySelector('.shape');
        p.textContent = 'Your body shape is Inverted Triangle In this body shape, the shoulders are wider than the hips, and the top part of the body looks bigger than the bottom. The bust and arms may look stronger or fuller, while the hips look narrower'
        ...
        const avoid = document.querySelector('.avoid');
        const shape = document.querySelector('.shape');
        p.textContent = 'Your body shape is Oval ( Apple ) In this body shape, most of the weight is in the middle of the body (stomach and waist). The chest may also look fuller, while the arms and legs often look slimmer. The body looks rounder at the center'
        ...
        const avoid = document.querySelector('.avoid');
        const shape = document.querySelector('.shape');
        p.textContent = 'Your body shape is Hourglass In this body shape, the shoulders and hips are almost the same size, and the waist is narrow and clearly seen. The body looks balanced and has natural curves, often called the “8 shape”'
        ...
        const avoid = document.querySelector('.avoid');
        const shape = document.querySelector('.shape');
        p.textContent = 'Your body shape is Rectangle In this body shape, the shoulders, waist, and hips are almost the same size. The waist is not very narrow, and there are fewer natural curves. The body often looks straight, flat, and athletic'
...
// event
btn.addEventListener('submit',bodyShapeCalculate);